...
contract Registry {
    ...
    */
    ...
    // 1 application
    // 2 challenge
    // 3 registered (whitelist)
    function getStatus(bytes32 _listing) constant public returns (uint status) {
        if (isWhitelisted(_listing)) {
            return 3;
        } else if (challengeExists(_listing)) {
            return 2;
        } else if (appWasMade(_listing)) {
            return 1;
        }
        return 0;
    }
