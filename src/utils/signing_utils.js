...
const signing_utils = {
  ...
  verifySignedMessage: async ({ address, message, signature, version }) => {
    const sig = new Buffer(signing_utils.stripHexPrefixAndLower(signature), 'hex')
    if (sig.length !== 65) {
      return false
    }
    ...
    return (
      signing_utils.stripHexPrefixAndLower(address) ===
  ...
  stripHexPrefixAndLower(value) {
    return signing_utils.stripHexPrefix(value).toLowerCase()
  },
  padLeftEven(hex) {
  ...
  sanitizeHex(hex) {
    ...
    return hex !== '' ? `0x${signing_utils.padLeftEven(hexStr)}` : ''
  },
